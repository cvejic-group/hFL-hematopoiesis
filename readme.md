...
This repository centralizes processing and analysis scripts accompanying the manuscript:

> **Temporal and regulatory landscape of human embryonic and foetal liver haematopoiesis mapped by single-nucleus multiomics**

## Codebase

Code and scripts were used for the custom analyses in the manuscript. These are shared to improve reproducibility of the main results, but the repository is not a complete, standalone workflow. Additional details are provided in the READMEs found in each subfolder.

### Cloning
Clone this repository (and submodule pipelines) with:

```bash
git clone --recurse-submodules https://github.com/cvejic-group/hFL-hematopoiesis.git
```
