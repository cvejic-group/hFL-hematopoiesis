...
```{r}
de_lmm <- readRDS(here::here("output", "cnmf_hsc.k5_dt001.G0_de", "G0_de.by_lmm.res_df.rds"))
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.GEP3_cmp_previous_qHSC_signatures.pdf"),
  width = 4, height = 3
)
```
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.umap_highlight_G0.pdf"),
  width = 3.5, height = 3.2
)
```

## Olp with G0 defined by Lisha
...
```{r fig.width=4, fig.height=3}
v = euler(c(A = 1889, B = 537, "A&B" = 1240))
plot(v, labels = c("G0_byGEP3", "G0_byBM_LT"),
     quantities = list(type = c("counts")), 
     legend = FALSE,
     fills = list(fill = c("blue", "yellow"), alpha = 0.5))

pdf(file = here::here("output", DOCNAME, "cNMF_HSC.cmp_G0_cells.pdf"), width = 4, height = 3)
plot(v, labels = c("G0_byGEP3", "G0_byBM_LT"),
     quantities = list(type = c("counts")), 
     legend = FALSE,
     fills = list(fill = c("blue", "yellow"), alpha = 0.5))
...
p <- DotPlot(srat, features = c(rev(c_markers), rev(q_markers)), group.by = 'PhaseNew',
             dot.scale = 3) +
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.QvsC.dotplot_known_markers.pdf"),
  width = 2.5, height = 5
)
```
...
VlnPlot(srat, features = c("ITGA6", "THY1", "CDK6", "CCND1", "CCND3"),
        group.by = "PhaseNew", pt.size = 0, ncol = 3) +
  NoLegend()
```
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.QvsC.composition_barplot.pdf"),
  width = 3, height = 5
)
```
...
### Olp with G0 defined by BM-LT signature

```{r fig.width=4, fig.height=3}
df_de_sig1 <- readRDS(here::here("output", "HSC.CvsQ.DE", "G0_de.by_lmm.res_df.rds")) |> 
  filter(ltsr > 0.9)

cmp_lst <- list(
  G0_BM_up = df_de_sig1 |> filter(beta > 0) |> pull(gene),
  G0_BM_dn = df_de_sig1 |> filter(beta < 0) |> pull(gene),
  G0_NMF_up = df_de_sig |> filter(beta > 0) |> pull(gene),
  G0_NMF_dn = df_de_sig |> filter(beta < 0) |> pull(gene)
)

padding = 0.4
p <- ggvenn(cmp_lst, set_name_size = 3, 
       text_size = 3,
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.QvsC.DEG_olp_with_G0_by_signature.pdf"),
  width = 4, height = 3
)
```
...
col_fun = circlize::colorRamp2(c(-0.2, 0, 0.3), c("blue", "white", "red"))
...
marker_2_lab_l = sapply(marker_2_lab_l, function(x){which(rownames(mat) == x)}, simplify = T)
ha_l = rowAnnotation(foo = anno_mark(at = marker_2_lab_l,
                                   labels = names(marker_2_lab_l),
                                   labels_gp = gpar(fontsize = 6)))

ht = Heatmap(mat, name = "beta",
        col = col_fun,
        right_annotation = ha_l,
        cluster_columns = F,
        cluster_rows = F,
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.QvsC.DEG_heatmap_show_known.pdf"),
  width = 1.35, height = 6
)
```
...
gene_df <- data.frame(
  gene = c("NKAIN2",
           "ATP10D", "ATP8A1", "ATP11A",
            "ATP6V0A1",
            "MT-ATP8", "ATP5PD", "ATP5PF", "ATP5MC1", "ATP5MPL", "ATP5F1E", "ATP5F1A", "ATP5F1B", "ATP5MG", "ATP5MC2", "ATP5MC3"),
  gene_group = c(rep("P-type ATPase Interacting", 1),
                 rep("P-type ATPase", 3),
                 rep("V-type ATPase", 1),
                 rep("F-type ATPase", 11)
                 )
)
...
# rowname anno
ha_r = rowAnnotation(foo = anno_mark(at = gth, labels = names(gth),
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.QvsC.DEG_heatmap_show_ATPase.pdf"),
  width = 3.5, height = 6
)
```

### heatmap of known and ATPase
...
#draw(ht, heatmap_legend_side = "bottom")
p.ht2 = plot_grid(grid.grabExpr(draw(ht, heatmap_legend_side = "top", annotation_legend_list = list(pd))))
p.ht2

ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.QvsC.DEG_heatmap_show_known_&_ATPase.pdf"),
  width = 4, height = 6
)
```
...
cg.BP <- readRDS(here::here("output", "cnmf_hsc.k5_dt001.G0_de", paste0(out_prefix, ".ORA_GOBP.rds")))
```
...
ggsave2(
  filename = here::here("output", DOCNAME, "cNMF_HSC.dev_de_plot.ORA_GO-BP.pdf"),
  width = 3.2, height = 6
)
```
