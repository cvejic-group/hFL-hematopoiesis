LFLMM <-
  function(Y, X, interactions=NULL, interactions3=NULL, a=rep(0,nrow(Y)), beta=NULL, psi=NULL, delta=NULL, Psi=NULL, theta=NULL, omega=NULL, subset=NULL, nLatent=0, forced=F, ITRMAX=30, PLOT=F, nexpcells=NULL, xgamma, s0=NULL, Hetero=""){
    ...
    if(!is.null(interactions)){
      for(j in seq(length(interactions))){
        Z.int=NULL
        for(i in interactions[[j]]){
          if(is.character(X[,i]) || is.factor(X[,i])){
            Z1 = array(0,c(sum(subset),length(table(X[subset,i]))))
            Z1[!is.na(X[subset,i]),] = model.matrix(~0+X[subset,i])
            colnames(Z1) = gsub("X\\[subset, i\\]","",colnames(model.matrix(~0+X[subset,i])))
          ...
          Z.int = c(Z.int, list(Z1))
        }
        P1 = ncol(Z.int[[1]])
        P2 = ncol(Z.int[[2]])
        Z1 = Z.int[[1]][,rep(1:P1,rep(P2,P1))] * Z.int[[2]][,rep(1:P2,P1)]
        colnames(Z1) = paste(colnames(Z.int[[1]])[rep(1:P1,rep(P2,P1))], colnames(Z.int[[2]])[rep(1:P2,P1)], sep=":")
        Z = cbind(Z, Z1[,colSums(Z1)!=0])
        nh = c(nh, sum(colSums(Z1)!=0))
        namesnh = c(namesnh, paste(interactions[[j]],collapse=":"))
      }
    }
    names(nh)=namesnh

    if(!is.null(interactions3)){
      for(j in seq(length(interactions3))){
        Z.int=NULL
        for(i in interactions3[[j]]){
          if(is.character(X[,i]) || is.factor(X[,i])){
            Z1 = array(0,c(sum(subset),length(table(X[subset,i]))))
            Z1[!is.na(X[subset,i]),] = model.matrix(~0+X[subset,i])
            colnames(Z1) = gsub("X\\[subset, i\\]","",colnames(model.matrix(~0+X[subset,i])))
        ...
        P2 = ncol(Z.int[[2]])
        P3 = ncol(Z.int[[3]])
        Z12 = Z.int[[1]][,rep(1:P1,rep(P2,P1))] * Z.int[[2]][,rep(1:P2,P1)]
        colnames(Z12) = paste(colnames(Z.int[[1]])[rep(1:P1,rep(P2,P1))], colnames(Z.int[[2]])[rep(1:P2,P1)], sep=":")
        P12=P1*P2
        Z123 = Z12[,rep(1:P12,rep(P3,P12))] * Z.int[[3]][,rep(1:P3,P12)]
        colnames(Z123) = paste(colnames(Z12)[rep(1:P12,rep(P3,P12))], colnames(Z.int[[3]])[rep(1:P3,P12)], sep=":")

        Z = cbind(Z, Z123[,colSums(Z123)!=0])
        nh = c(nh, sum(colSums(Z123)!=0))
        namesnh = c(namesnh, paste(interactions3[[j]],collapse=":"))
      }
    }
    names(nh)=namesnh
...
getBF <-
  function(Y, res.nlm, colname, ncont=2, Resid=0, geta=100, Comprehensive=T, DE1=F){
    ...
    if(Mk>2){if(Comprehensive){z = emn(BF)}else{z = emn(BF[,1:length(CON)])}}else{z=emn(BF[,1,drop=F])}
...
getBFInt <-
  function(Y, res.nlm, colname, Celltype, ncont=2, Resid=0, geta=100, Comprehensive=F, DE1=T){
    ...
    h1 = (seq(M)%in%grep(paste("^",gsub("\\+","\\\\+",gsub("\\(","\\\\(",gsub("\\)","\\\\)",Celltype))),":",sep=""),colnames(res.nlm$Z))) * H[,k0]
    H[,k0]=H[,k0]-h1
    H = cbind(H, h1)
    K=K+1
    k=K
    ...
    if(Mk==1 || is.na(DE1)){
      BF[,1] = (apply((YZ%*%solve(A))*YZ,1,sum) - apply((YZ0%*%solve(A0))*YZ0,1,sum))/2/s2 - logDet(diag(M)+t(t(ZtZ)*u)*u)/2 + logDet(diag(M0)+(t(t(ZtZ0)*u0)*u0))/2
      if(Mk==1)DB[,1] = (YZ%*%solve(A))[,H[,k]==1]
      if(Mk==1)DBSE[,1] = sqrt(solve(A)[H[,k]==1,H[,k]==1]*s2)
    }else{
      BF[,length(CON)+1] = (apply((YZ%*%solve(A))*YZ,1,sum) - apply((YZ0%*%solve(A0))*YZ0,1,sum))/2/s2 - logDet(diag(M)+t(t(ZtZ)*u)*u)/2 + logDet(diag(M0)+(t(t(ZtZ0)*u0)*u0))/2
      for(l in 1:length(CON)){
        print(l)
    ...
    if(Mk==1 || is.na(DE1)){
      ...
      if(Comprehensive){
        ...
        z = emn(BF[,1:length(CON)])
      }
    }
    ltsr = ltsr*z[,1:ncol(ltsr)]

    if(Mk>1 && !is.na(DE1))colnames(ltsr) = getContrastLevels(list(contrasts=CON,levels=levelnames))
    colnames(B) = levelnames
    rownames(ltsr) = rownames(B)
    if(Mk>1 && !is.na(DE1))colnames(DBSE) = getContrastLevels(list(contrasts=CON,levels=levelnames))
    rownames(DBSE) = rownames(B)
...
BarplotGG <- function(x, col = "blue") {
  ...
  data = data.frame(
    name = factor(labs[ord], levels = labs[ord]),
    varexp = varexp,
    varse = varse
  )
