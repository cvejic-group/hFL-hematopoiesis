...
## Cmp with LMM-RNA {.tabset}

change of peaks near gene promoter VS. change of gene expression

Load LMM rna

```{r}
de_rna <- readRDS(here::here("output", "HSC.dev_de_lmm", "HSC_lmm_de_time_df.rds")) |> 
  dplyr::rename(beta_rna = beta, ltsr_rna = ltsr)
dim(de_rna)
```
...
da_atac_pro <- as.data.frame(peakAnnoAll@anno) |> 
  # 1kb to TSS
  filter(abs(distanceToTSS) <= 1000, !is.na(geneId)) |>
  dplyr::select(peak = V4, beta_atac = V5, ltsr = V6, geneId, ENTREZID, gene = SYMBOL, distanceToTSS) |> 
  # for each gene, use the beta of closest peak
  group_by(geneId) |> 
  top_n(1, wt = -abs(distanceToTSS)) |> 
  arrange(-beta_atac)

d4p <- de_rna |> 
  filter(ltsr_rna > 0.9) |> 
  left_join(da_atac_pro, by = "gene") |> 
  filter(!is.na(beta_atac))
dim(d4p)

d4p |> 
  ggplot(aes(x = beta_rna, y = beta_atac)) +
  geom_point_rast() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  stat_cor() +
  geom_text_repel(aes(label = gene), max.overlaps = 10) +
  labs(x = "LMM-RNA Beta", y = "LMM-ATAC Beta of closest peak to TSS", title = "Sig. in LMM-RNA")
```

### sig. in ATAC

```{r fig.width=6, fig.height=5}
da_atac_pro_sig <- as.data.frame(peakAnnoAll@anno) |> 
  # 1kb to TSS
  filter(abs(distanceToTSS) <= 1000, !is.na(geneId)) |>
  dplyr::select(peak = V4, beta_atac = V5, ltsr_atac = V6, geneId, ENTREZID, gene = SYMBOL, distanceToTSS) |> 
  # for each gene, use the beta of closest peak
  group_by(geneId) |> 
  top_n(1, wt = -abs(distanceToTSS)) |> 
  arrange(-beta_atac) |> 
  filter(ltsr_atac > 0.9)

d4p <- da_atac_pro_sig |> 
  left_join(de_rna, by = "gene") |> 
  filter(!is.na(beta_rna))
dim(d4p)
...
d4p |> 
  ...
  stat_cor() +
  geom_text_repel(aes(label = gene), max.overlaps = 10) +
  labs(x = "LMM-RNA Beta", y = "LMM-ATAC Beta of closest peak to TSS", title = "Sig. in LMM-ATAC")
```
...
d4p <- de_rna |> 
  filter(ltsr_rna > 0.9) |> 
  left_join(da_atac_pro_sig, by = "gene") |> 
  filter(!is.na(beta_atac))
dim(d4p)
...
d4p |> 
  ...
  stat_cor() +
  geom_text_repel(aes(label = gene), max.overlaps = 10) +
  labs(x = "LMM-RNA Beta", y = "LMM-ATAC Beta of closest peak to TSS")
```
...
## GSEA of gene promoter overlap with DA peaks {.tabset}

Too many genes. Let's check genes whose promoters overlap with peaks.

```{r, eval=FALSE}
tmp_df <- na.omit(da_atac_pro[,c("beta_atac", "ENTREZID")])
geneList <- setNames(tmp_df$beta_atac, tmp_df$ENTREZID)
...
saveRDS(gse_kg, file = here::here("output", DOCNAME, "HSC_lmm_da_time.GSEA_KEGG.rds"))
as.data.frame(gse_kg) |> 
  write_tsv(here::here("output", DOCNAME, "HSC_lmm_da_time.GSEA_KEGG.tsv"))
...
gse_go <- gseGO(geneList     = geneList,
               ...
               nPermSimple = 10000,
               verbose      = FALSE)
gse_go <- setReadable(gse_go, 'org.Hs.eg.db', keyType = "ENTREZID")
saveRDS(gse_go, file = here::here("output", DOCNAME, "HSC_lmm_da_time.GSEA_GOBP.rds"))
as.data.frame(gse_go) |> 
  write_tsv(here::here("output", DOCNAME, "HSC_lmm_da_time.GSEA_GOBP.tsv"))
```

KEGG

```{r}
gse_kg <- readRDS(here::here("output", DOCNAME, "HSC_lmm_da_time.GSEA_KEGG.rds"))
...
```{r}
gse_go <- readRDS(here::here("output", DOCNAME, "HSC_lmm_da_time.GSEA_GOBP.rds"))
...
## ORA of gene promoter overlap with DA peaks {.tabset}

Too many genes. Let's check genes whose promoters overlap with peaks.

```{r, eval=FALSE}
genes.pro = list(
  peak_up = da_atac_pro_sig |> filter(!is.na(ENTREZID), beta_atac > 0) |> pull(ENTREZID),
  peak_dn = da_atac_pro_sig |> filter(!is.na(ENTREZID), beta_atac < 0) |> pull(ENTREZID)
)

# KEGG
ck.pro <- compareCluster(genes.pro, fun = "enrichKEGG", organism = "hsa",
                         pvalueCutoff  = 0.05, pAdjustMethod = "BH")
ck.pro <- setReadable(ck.pro, 'org.Hs.eg.db', keyType = "ENTREZID")
saveRDS(ck.pro, file = here::here("output", DOCNAME, "HSC_lmm_da_time.ORA_KEGG.rds"))
as.data.frame(ck.pro) |> 
  write_tsv(here::here("output", DOCNAME, "HSC_lmm_da_time.ORA_KEGG.tsv"))

# GOBP
cg.BP.pro = compareCluster(genes.pro, fun = "enrichGO",
                           OrgDb = org.Hs.eg.db, ont = "BP", pAdjustMethod = "BH",
                           pvalueCutoff = 0.01, qvalueCutoff = 0.05, readable = T)
saveRDS(cg.BP.pro, file = here::here("output", DOCNAME, "HSC_lmm_da_time.ORA_GOBP.rds"))
as.data.frame(cg.BP.pro) |> 
  write_tsv(here::here("output", DOCNAME, "HSC_lmm_da_time.ORA_GOBP.tsv"))
```

### KEGG

```{r, fig.width=8, fig.height=7}
ck.pro <- readRDS(here::here("output", DOCNAME, "HSC_lmm_da_time.ORA_KEGG.rds"))

dotplot(ck.pro, showCategory = 10, title = "KEGG")
```

### GO-BP

```{r, fig.width=8, fig.height=8}
cg.BP.pro <- readRDS(here::here("output", DOCNAME, "HSC_lmm_da_time.ORA_GOBP.rds"))

dotplot(cg.BP.pro, showCategory = 10, title = "GO:BP") +
