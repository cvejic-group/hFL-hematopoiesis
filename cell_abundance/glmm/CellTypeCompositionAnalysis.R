# most functions from: https://github.com/Teichlab/SKM_ageing_atlas/blob/main/Figures/Figure1/Ctype_abundance_analyses_Fig1d_and_Supplementary.ipynb
# and: https://github.com/123anjuan/HMA/blob/main/block%201/03.figure1d_LTSR.R
      ...
      lapply(names(vars), function(vname) {
        if (vname %in% names(references)) {
          ...
          full_join(
            ...
            cat_ranef_tbl %>% filter(grpvar == vname, grpval == ref) %>% dplyr::select(grpvar, Celltype, condval, condsd),
            ...
            dplyr::select(-condval.x, -condval.y, -condsd.x, -condsd.y)
        } else {
...
# remove facet, modified based on function above
plot_ranef2 <- function(ranef_tbl, vars, celltypes = NULL, celltype_order = "hclust", references = NULL,
                       maxFC = 3, LTSR2p = F, highlightLtsr = 0.0, filterLtsr = 0.0, swap_axes = F) {
  ranef_tbl <- .getCondValLtsr(ranef_tbl, vars, celltypes = celltypes, references = references)
...
CellTypeCompositionAnalysis2 <- function(f=NULL, data=NULL){
  res.prop <- glmer(f, data = data, family=poisson,
                  control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  # standard errors of standard deviations (squre root of the variance parameters)]
  devfun = update(res.prop, devFunOnly=T)
  pars = getME(res.prop, c("theta", "fixef"))
