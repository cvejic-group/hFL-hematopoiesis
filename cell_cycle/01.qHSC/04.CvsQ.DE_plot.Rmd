...
d4p <- srat@meta.data |> filter(PCW_new != 'Mixed') |> 
  droplevels() |> 
  group_by(PCW_new, G0_label) |> 
  summarize(count = n()) |>
  mutate(prop = count / sum(count) * 100)

p <- d4p |> 
  mutate(PCW_new = factor(PCW_new, levels = as.character(18:5))) |> 
  ggplot(aes(x = PCW_new, y = prop, fill = G0_label)) +
  geom_col() +
...
```{r}
d4p <- srat@meta.data |> filter(PCW_new != 'Mixed') |> 
  group_by(PCW_new, donorID, G0_label) |> 
  summarise(n = n()) |> 
  mutate(perc = n/sum(n)*100)

d4p |> 
  ggplot(aes(x = donorID, y = perc, fill = G0_label)) +
  geom_col() +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_manual(values = cycle2col) +
  facet_grid(.~PCW, scales = "free_x", space = "free_x") +
  labs(x = NULL, y = "Percentage", fill = "CC Phase") +
  theme(
...
ggsave2(
  filename = here::here("output", DOCNAME, "HSC.QvsC.DEG_heatmap_show_known_&_ATPase.pdf"),
  width = 4, height = 6
)
```
